...
# Roadmap Stage 12 Diagnostics

## Scope

Goal of this roadmap stage: polish user-facing subscription/help copy without changing callback_data, button labels, or button order.

## Added

- `bot/services/subscription_view_service.py`
- updated `bot/ui_user.py`
- updated `bot/handlers/user_render.py`
- updated `tests/test_ui_contracts.py`
- added `tests/test_subscription_view_service.py`

## Current implementation

- main menu subscription status lines are generated by `subscription_view_service.build_main_menu_status_lines(...)`
- help text is generated by `subscription_view_service.build_help_text(...)`
- invoice sent / balance purchase notices are generated through the same service
- visible UX now explicitly explains remaining time, next step after payment, and pending join auto-approval

## Contract boundaries preserved

- user `callback_data` unchanged
- button texts unchanged
- button order unchanged
- payment payload unchanged
- no payment/access/maintenance logic changes

## Tests added

Coverage added:

- active subscription status lines
- inactive pending-join status lines
- post-payment help copy
- user main menu text contracts
- help text contract while preserving keyboard layout
